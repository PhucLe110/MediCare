...
const getDoctorProfileByUserId = async (userId) => {
  const doctor = await Doctor.findOne({ userId }).populate(
    "userId",
    "fullName email phone gender status",
  );
...
const startExamination = async (appointmentId) => {
  ...
  if (
    ...
    throw new HttpError(
      400,
      "Không thể bắt đầu ca khám đã hoàn thành hoặc đã hủy",
    );
  }
  if (!isAppointmentTimeReached(appointment)) {
    throw new HttpError(
      400,
      "Chưa đến giờ khám. Vui lòng bắt đầu khi đến đúng giờ hẹn của bệnh nhân.",
    );
  }
...
const completeDiagnosis = async (
  appointmentId,
  doctorUserId,
  { diagnosis, doctorNotes, medicines, followUp },
) => {
  const appointment = await Appointment.findById(appointmentId);
  if (!appointment) throw new HttpError(404, "Không tìm thấy ca khám");
  ...
  if (diagnosis || (medicines && medicines.length > 0)) {
    ...
    if (medicines?.length > 0) {
      for (const med of medicines) {
        ...
        if (updated && updated.stock >= 10 && updated.status === "low") {
...
const createShiftRequest = async (userId, { type, date, times }) => {
  ...
  if (!times?.length)
    throw new HttpError(400, "Vui lòng chọn ít nhất một khung giờ");
  if (getDayOfWeek(date) === 0) {
    throw new HttpError(400, "Bệnh viện không làm việc Chủ Nhật");
  }

  const currentTimes = await getEffectiveTimesForDate(
    doctorProfile._id,
    date,
    doctorProfile,
    "shift",
  ...
  if (type === "add") {
    ...
    if (conflicts.length > 0) {
      throw new HttpError(
        400,
        `Các khung giờ sau đã có trong lịch trực: ${conflicts.join(", ")}. Không thể thêm trùng.`,
      );
    }
  }
  ...
  if (type === "cancel") {
    ...
    if (notExist.length > 0) {
      throw new HttpError(
        400,
        `Các khung giờ sau không có trong lịch trực ngày này: ${notExist.join(", ")}. Không thể hủy.`,
      );
    }
  }
...
const getMyShiftRequests = async (userId) => {
  const doctorProfile = await getDoctorProfileByUserId(userId);
  return ShiftRequest.find({ doctor: doctorProfile._id }).sort({
    createdAt: -1,
  });
};
