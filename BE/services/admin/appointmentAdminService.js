...
const { MAX_PATIENTS_PER_SLOT, RESCHEDULE_MAX_PER_DOCTOR } = require('../../constants/appointment');
...
const getAvailableDoctorsForReschedule = async (id, query) => {
  ...
  if (!currentDoctor) throw new HttpError(400, 'Ca khám này chưa có bác sĩ phụ trách');
...
const rescheduleAppointment = async (id, body) => {
  ...
  if (apptCount >= MAX_PATIENTS_PER_SLOT) {
    throw new HttpError(400, 'Khung giờ của bác sĩ này đã vượt quá giới hạn 5 bệnh nhân!');
  }
