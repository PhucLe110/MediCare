...
exports.createLabRequest = asyncHandler(async (req, res) => {
  ...
  res
    ...
    .json({
      success: true,
      message: "Đã gửi yêu cầu xét nghiệm thành công!",
      data,
    });
});
...
exports.getAllRequests = asyncHandler(async (req, res) => {
  console.log(
    "getAllRequests called, user:",
    req.user?._id,
    "role:",
    req.user?.role,
  );
  const data = await labRequestService.getAllRequests(
    req.user,
...
exports.cancelLabRequest = asyncHandler(async (req, res) => {
  const data = await labRequestService.cancelLabRequest(
    req.params.id,
    req.user,
  );
  res
    .status(200)
    .json({ success: true, message: "Đã hủy yêu cầu xét nghiệm.", data });
});
