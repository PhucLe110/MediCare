  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const unpaid = br.success
          ? br.data.filter((b) => b.status === "unpaid").length
          : 0;
        const pending = ar.success
          ? ar.data.filter((a) => a.status === "pending").length
          : 0;
    ...
    <div className="flex min-h-screen bg-gray-50 font-sans transition-colors duration-200">
      ...
      <aside className="w-64 bg-primary text-white flex flex-col fixed h-full z-20 shadow-xl shadow-blue-900/10">
        <div className="p-6">
          <Link
            to="/dashboard"
            className="flex items-center justify-center cursor-pointer mb-8"
          >
            <img
              src="/LOGO.png"
              alt="MediCare"
          ...
          <div className="mb-6 bg-white/10 p-4 rounded-2xl border border-white/10 shadow-inner">
            {/* Language & Theme Switchers - Separated at two ends */}
            <div className="flex justify-around mb-3">
              {/* Language Switcher - Left */}
              <div className="flex bg-white/5 p-1 rounded-lg border border-white/10 shadow-inner gap-0.5">
                <button
                  onClick={() => setLang("vi")}
                  className={`w-7 h-7 flex items-center justify-center rounded-md text-[10px] font-black tracking-wider transition-all ${lang === "vi" ? "bg-white text-primary shadow-md" : "text-blue-200 hover:text-white"}`}
                >
                  VI
                </button>
                <button
                  onClick={() => setLang("en")}
                  className={`w-7 h-7 flex items-center justify-center rounded-md text-[10px] font-black tracking-wider transition-all ${lang === "en" ? "bg-white text-primary shadow-md" : "text-blue-200 hover:text-white"}`}
                >
                  EN
                </button>
              </div>
              ...
              <div className="flex bg-white/5 p-1 rounded-lg border border-white/10 shadow-inner gap-0.5">
                <button
                  ...
                  className={`w-7 h-7 rounded-md transition-all flex items-center justify-center ${theme === "light" ? "bg-white text-primary shadow-md" : "text-blue-200 hover:text-white"}`}
                  title={t.light}
                >
                  <Sun size={14} />
                </button>
                <button
                  ...
                  className={`w-7 h-7 rounded-md transition-all flex items-center justify-center ${theme === "dark" ? "bg-white text-primary shadow-md" : "text-blue-200 hover:text-white"}`}
                  title={t.dark}
                >
                  <Moon size={14} />
                </button>
