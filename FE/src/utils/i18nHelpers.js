...
export const getDefaultLabTests = (lang) =>
  DEFAULT_LAB_TEST_NAMES[lang] || DEFAULT_LAB_TEST_NAMES.vi;
...
export const resolveDeptLabel = (lang, dept, variant = "default") => {
  ...
  // Chỉ match chính xác "Nội tổng quát" hoặc "General Internal Medicine", không match "Ngoại tổng quát"
  if (
    dLower === "nội tổng quát" ||
    dLower === "khoa nội tổng quát" ||
    dLower === "general internal medicine"
  ) {
    if (variant === "landing")
      return pickLang(lang, "Khám tổng quát", "General Practice");
    if (variant === "doctors")
      return pickLang(lang, "Khoa Nội tổng quát", "General Practice");
  }
...
export const resolveSpecialtyLabel = (lang, spec) => {
  if (!spec) return "";
  // Trả về tên chuyên khoa gốc, không thay đổi
  return spec;
};

export const otherDeptLabel = (lang) =>
  pickLang(lang, "Chuyên Khoa Khác", "Other Specialties");
