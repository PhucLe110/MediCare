    ...
    <div className="max-w-5xl mx-auto">
      <div className="flex items-center gap-4 mb-8 bg-[var(--card-bg)] p-6 rounded-3xl shadow-sm border border-[var(--border-color)]">
        <div className="relative">
          <div className="w-16 h-16 bg-gradient-to-br from-blue-100 dark:from-blue-900/30 to-indigo-100 dark:to-indigo-900/30 text-primary rounded-2xl flex items-center justify-center shadow-inner relative z-10">
            <Bot size={32} />
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-[var(--card-bg)] p-8 rounded-3xl shadow-sm border border-[var(--border-color)] flex flex-col h-full relative overflow-hidden">
          <div className="absolute top-0 right-0 w-32 h-32 bg-blue-50 dark:bg-blue-900/20 rounded-full blur-3xl -z-10"></div>
          ...
          <div className="relative flex-1 flex flex-col">
            <textarea
              className={`w-full flex-1 min-h-[200px] p-5 bg-[var(--bg-tertiary)] border ${error ? "border-red-300 dark:border-red-900/30 focus:ring-red-200" : "border-[var(--border-color)] focus:border-primary/50 focus:ring-primary/10"} rounded-2xl text-sm focus:outline-none focus:ring-4 transition-all resize-none shadow-inner text-[var(--text-primary)]`}
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              <span className="text-xs text-[var(--text-tertiary)] font-medium w-full mb-1">
                {t.quickSugg}
              </span>
              {t.suggestions.map((suggestion) => (
                <button
                  ...
                  className="px-3 py-1.5 bg-[var(--card-bg)] border border-[var(--border-color)] text-[var(--text-secondary)] text-xs rounded-full hover:border-primary hover:text-primary transition-all shadow-sm"
                >
                  {suggestion}
                </button>
              ))}
          ...
          {error && (
            <p className="text-red-500 text-sm mt-4 font-medium animate-in fade-in">
              {error}
            </p>
          )}
          ...
          <div className="flex gap-3 mt-6">
            ...
            {result && (
              <button
                onClick={resetForm}
                className="p-4 bg-[var(--bg-tertiary)] border border-[var(--border-color)] text-[var(--text-secondary)] rounded-xl hover:bg-[var(--border-color)] hover:text-primary transition-all tooltip"
        ...
        <div className="h-full min-h-[400px]">
          {!result && !loading && (
            <div className="h-full bg-gradient-to-br from-blue-50/50 dark:from-blue-900/20 to-indigo-50/50 dark:to-indigo-900/20 border border-blue-100/50 dark:border-blue-900/30 rounded-3xl p-8 flex flex-col items-center justify-center text-center">
              <div className="w-24 h-24 bg-[var(--card-bg)] rounded-full flex items-center justify-center shadow-sm mb-6 border border-blue-50 dark:border-blue-900/30">
          ...
          {loading && (
            <div className="h-full bg-[var(--card-bg)] border border-[var(--border-color)] rounded-3xl p-8 flex flex-col items-center justify-center text-center shadow-sm">
              <div className="relative mb-8">
                <div className="w-24 h-24 bg-blue-100 dark:bg-blue-900/30 rounded-full animate-ping absolute opacity-40"></div>
          ...
          {result && !loading && (
            <div className="bg-[var(--card-bg)] rounded-3xl shadow-xl shadow-blue-900/5 border border-[var(--border-color)] overflow-hidden animate-in slide-in-from-right-8 duration-500 h-full flex flex-col relative group">
              ...
              <div className="p-8 relative z-10 border-b border-[var(--border-color)]">
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 bg-blue-100 dark:bg-blue-900/30 text-primary rounded-xl flex items-center justify-center">
                      <Activity size={24} />
                    </div>
                    <div>
                  ...
                  <span
                    className={`px-4 py-2 text-xs font-black uppercase rounded-xl border tracking-wider shadow-sm ${
              ...
              <div className="p-8 flex-1 flex flex-col relative z-10 bg-[var(--card-bg)]/80 backdrop-blur-sm">
                <div className="flex-1">
                  ...
                  {result.matchedKeywords.length > 0 && (
                    <div className="mb-6 p-5 bg-[var(--bg-tertiary)] rounded-2xl border border-[var(--border-color)]">
                      <p className="text-xs text-[var(--text-secondary)] font-bold uppercase tracking-wider mb-3 flex items-center gap-2">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {result.matchedKeywords.map((kw, idx) => (
                          <span
                            key={idx}
                            className="px-3 py-1.5 bg-[var(--card-bg)] text-accent text-xs font-bold rounded-xl border border-red-100 dark:border-red-900/30 shadow-sm"
                          >
                            {kw}
                          </span>
                        ))}
                  ...
                  <div className="mt-4 p-4 rounded-xl flex items-start gap-3 bg-blue-50/50 dark:bg-blue-900/20">
                ...
                <button
                  ...
                  className="w-full mt-8 py-4 bg-gray-900 dark:bg-[var(--text-primary)] text-white rounded-xl font-bold hover:bg-black dark:hover:bg-[var(--text-primary)] transition-all shadow-xl shadow-gray-900/20 flex justify-center items-center gap-2 group overflow-hidden relative"
                >
