...
const AITriage = () => {
  ...
  const handlePredict = async () => {
    if (!symptoms.trim()) {
      setError('Vui lòng nhập triệu chứng của bạn để AI phân tích.');
      return;
    }
    ...
    try {
      ...
      console.error(err);
      setError('Đã xảy ra lỗi hệ thống khi kết nối đến AI.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="flex items-center gap-4 mb-8 bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-800 bg-clip-text text-transparent bg-gradient-to-r from-gray-800 to-primary">AI Tư Vấn Sức Khỏe</h1>
          <p className="text-gray-500 mt-1">Trợ lý y tế ảo giúp bạn phân tích triệu chứng và tìm đúng chuyên khoa</p>
        </div>
      </div>
        ...
        <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 flex flex-col h-full relative overflow-hidden">
          ...
          <div className="relative flex-1 flex flex-col">
            <textarea
              ...
              placeholder="Ví dụ: Tôi bị đau đầu dữ dội 2 ngày nay, kèm theo chóng mặt khi đứng lên ngồi xuống và hơi buồn nôn vào buổi sáng..."
              value={symptoms}
              onChange={(e) => {
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              <span className="text-xs text-gray-400 font-medium w-full mb-1">Gợi ý nhanh:</span>
              {['Đau đầu, sốt ho', 'Đau dạ dày, buồn nôn', 'Sốt cao, phát ban', 'Đau mỏi vai gáy'].map(suggestion => (
                <button 
                  key={suggestion}
          ...
          <div className="flex gap-3 mt-6">
            ...
            >
              {loading ? (
                <><Loader2 size={20} className="animate-spin" /> Đang phân tích dữ liệu...</>
              ) : (
                <><Activity size={20} /> Phân tích triệu chứng</>
              )}
            </button>
        ...
        <div className="h-full min-h-[400px]">
          {!result && !loading && (
            <div className="h-full bg-gradient-to-br from-blue-50/50 to-indigo-50/50 border border-blue-100/50 rounded-3xl p-8 flex flex-col items-center justify-center text-center">
              ...
              <h3 className="text-xl font-bold text-gray-500 mb-2">AI Đang Chờ</h3>
          ...
          {loading && (
            <div className="h-full bg-white border border-gray-100 rounded-3xl p-8 flex flex-col items-center justify-center text-center shadow-sm">
              ...
              <h3 className="text-xl font-bold text-gray-800 mb-2">Hệ thống đang xử lý...</h3>
              <p className="text-sm text-gray-500">Thuật toán đang bóc tách từ khóa và đánh giá trọng số bệnh lý.</p>
            </div>
          )}
            ...
            <div className="bg-white rounded-3xl shadow-xl shadow-blue-900/5 border border-gray-100 overflow-hidden animate-in slide-in-from-right-8 duration-500 h-full flex flex-col relative group">
              ...
              <div className="p-8 relative z-10 border-b border-gray-50">
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Khoa Đề Xuất</p>
                      <h3 className="text-2xl font-black text-gray-800">{result.department}</h3>
                    </div>
                  </div>
                  <span className={`px-4 py-2 text-xs font-black uppercase rounded-xl border tracking-wider shadow-sm ${
                    ...
                    {result.confidence === 'high' ? 'ĐỘ CHÍNH XÁC CAO' : result.confidence === 'medium' ? 'ĐỘ CHÍNH XÁC KHÁ' : 'CẦN KHÁM TỔNG QUÁT'}
                  </span>
                </div>
                ...
                <div className="bg-primary text-white p-4 rounded-2xl shadow-inner relative overflow-hidden">
                  ...
                  <p className="text-sm font-medium text-blue-100 mb-1">Chuyên khoa cụ thể:</p>
                  <p className="text-xl font-bold relative z-10">{result.specialty}</p>
                </div>
              </div>
              ...
              <div className="p-8 flex-1 flex flex-col relative z-10 bg-white/80 backdrop-blur-sm">
                  ...
                  {result.matchedKeywords.length > 0 && (
                    <div className="mb-6 p-5 bg-gray-50 rounded-2xl border border-gray-100">
                      <p className="text-xs text-gray-500 font-bold uppercase tracking-wider mb-3 flex items-center gap-2">
                        <Activity size={14} /> Điểm nhấn triệu chứng:
                      </p>
                ...
                >
                  ...
                  Tiến hành Đặt lịch chuyên khoa này
