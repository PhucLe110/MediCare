...
const trans = {
  vi: {
    ...
    subtitle:
      "Quản lý mô hình gợi ý chẩn đoán, lịch sử phân tích và cập nhật tập dữ liệu huấn luyện.",
  ...
  en: {
    ...
    subtitle:
      "Manage diagnostic suggestion models, analysis history, and training dataset updates.",
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-4 bg-[var(--card-bg)] p-4 md:p-6 rounded-2xl md:rounded-3xl border border-[var(--border-color)] shadow-sm">
        ...
        <button
          type="button"
          className="px-4 md:px-5 py-2 md:py-2.5 bg-indigo-600 text-white font-bold rounded-xl flex items-center gap-2 hover:bg-indigo-700 shadow-lg shadow-indigo-600/20 text-xs md:text-sm"
        >
          <RefreshCw size={16} md={18} /> {t.retrain}
        </button>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
        <div className="bg-gradient-to-br from-indigo-900 to-slate-900 text-white p-5 md:p-8 rounded-2xl md:rounded-3xl shadow-lg relative overflow-hidden md:col-span-2">
          <div className="absolute right-0 top-0 opacity-10">
            <Bot size={150} md={200} />
          </div>
          <div className="relative z-10">
            <h3 className="text-lg md:text-xl font-black mb-4 md:mb-6">
              {t.modelStatus}
            </h3>
            <div className="grid grid-cols-2 gap-4 md:gap-6">
              ...
              <div>
                ...
                <p className="font-mono text-base md:text-xl font-bold text-emerald-400">
                  0.968
                </p>
              </div>
              <div>
                ...
                <p className="font-mono text-sm md:text-lg font-bold">
                  {t.lastUpdateVal}
                </p>
              </div>
              <div>
        ...
        <div className="bg-slate-900 dark:bg-slate-950 rounded-2xl md:rounded-3xl p-4 md:p-6 font-mono text-[10px] md:text-xs text-green-400 overflow-hidden shadow-lg border border-slate-800 dark:border-slate-800">
          <div className="flex items-center gap-2 mb-3 md:mb-4 text-slate-400 border-b border-slate-800 pb-2">
            <Terminal size={12} md={14} /> {t.logsTitle}
          </div>
