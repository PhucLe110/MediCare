  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Revenue Chart */}
        <div className="bg-[var(--card-bg)] rounded-3xl p-6 shadow-sm border border-[var(--border-color)] lg:col-span-2 relative overflow-hidden">
          <div className="flex justify-between items-center mb-6">
            ...
            <select className="bg-[var(--bg-tertiary)] border border-[var(--border-color)] text-sm font-bold text-[var(--text-primary)] rounded-xl px-4 py-2 outline-none">
        ...
        <div className="bg-[var(--card-bg)] rounded-3xl p-6 shadow-sm border border-[var(--border-color)]">
          ...
          <div className="h-64">
            {localizedDeptData.length === 0 ? (
              <div className="text-center py-20 text-[var(--text-tertiary)] font-medium">
                {t.deptFreqEmpty}
              </div>
            ) : (
              <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-gradient-to-br from-indigo-900 to-slate-900 rounded-3xl p-8 shadow-lg text-white relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <p className="text-indigo-200 font-medium mb-8 max-w-sm">
              {t.aiStatusSub}
            </p>
        ...
        <div className="bg-[var(--card-bg)] rounded-3xl p-6 shadow-sm border border-[var(--border-color)]">
          <div className="flex justify-between items-center mb-6">
            ...
            <button
              ...
              className="text-xs font-bold text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 bg-indigo-50 dark:bg-indigo-900/30 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 px-3 py-1.5 rounded-lg transition-all"
            >
              {t.docPerfViewAll}
            </button>
          </div>
          <div className="space-y-4">
            {doctorPerformances.length === 0 ? (
              <div className="text-center py-10 text-[var(--text-tertiary)] font-medium">
                {t.docPerfEmpty}
              </div>
            ) : (
              doctorPerformances.map((doc, idx) => (
                <div
                  key={idx}
                  className="flex items-center justify-between p-4 rounded-2xl border border-[var(--border-color)] hover:border-[var(--border-color)] transition-colors bg-[var(--bg-tertiary)]"
                >
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-full bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 flex items-center justify-center font-black text-sm">
                      {t.docTitle}
                    </div>
                    <div>
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-xs font-bold text-indigo-500 dark:text-indigo-400 flex items-center justify-end gap-1">
                      <ArrowUpRight size={12} /> {t.docPerfSpecialtyLabel}
                    </p>
                  </div>
                </div>
              ))
...
function StatCard({ title, value, icon: Icon, trend, isPositive, color }) {
  ...
  return (
    <div className="bg-[var(--card-bg)] rounded-3xl p-6 shadow-sm border border-[var(--border-color)] relative overflow-hidden group hover:shadow-md transition-shadow">
      <div
        className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-4 ${colorMap[color]}`}
      >
        <Icon size={26} />
      </div>
      <div>
        <h3 className="text-[var(--text-secondary)] font-bold text-sm mb-1">
          {title}
        </h3>
        <div className="flex items-end gap-3">
          ...
          <span
            className={`flex items-center gap-1 text-sm font-bold pb-1 ${isPositive ? "text-emerald-500 dark:text-emerald-400" : "text-red-500 dark:text-red-400"}`}
          >
            {isPositive ? (
